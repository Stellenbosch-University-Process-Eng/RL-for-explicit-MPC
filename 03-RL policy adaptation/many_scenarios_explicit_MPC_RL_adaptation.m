%% Script used to train RL agents warm started using explicit MPC.
%% Name: Edward Bras
%% Date: 2024-04-26

%% NOTE: Two structures must be available in the Current Folder:
%% 1) Structure containing warm-started policy - generated by "NN_fit_to_optimal_policy.m"
%% 2) Structure containing warm-started value function - generated by "NN_fit_to_optimal_value_function.m"
...
% specify problem size and reporting intervals
nmberStepsSpecified = 2e6;                  % number of training interactions per scenario
upperReportVec = nmberStepsSpecified;       % upper bound of training interactions printed to Command Window
upperPolicySavingVec = nmberStepsSpecified; % upper bound of training interactions for which policies are provided as simulation output
...
for scenarioCntr = 1:1:numberScenarios
    ...
    % NOTE: Commented code below can be used to initialise the critic
    % randomly in a separate script to generate a variable
    % "critic_scratch_NN.mat" mentioned in the earlier comment in this
    % script.
...
%     critic.NN.output_layer_parameters(1,:) = 0;
    ...
    [~,Output] = ode23s(@(t,x) myQTDEs(t,x,param,[v_1_SS,v_2_SS]),tspan,[h_1_SS_initial_guess,h_2_SS_initial_guess,h_3_SS_initial_guess,h_4_SS_initial_guess]');
...
function [out_agentExperience,outPol,p,outCrit] = trainFunction(actor,...
    ...
    for epCntr = 1:1:1 % start loop through episodes (outer loop of earlier code maintained, but there are no episodes here)
        ...
        for stepCntr = 1:1:p.nmberOfSteps
            ...
            [actor.NN,p] = ActorbackProp_ReLU_tanh_EL_traces(actor.NN,S_1_s,S_2_s,S_3_s,S_4_s,S_5_s,S_6_s,temporal_diff,u_1_s,u_2_s,p); % backpropagation function updated so that action applied is used during the backward pass
...
function [NN,temporal_diff,p] = RBF_TD_0(NN,State_1,State_2,State_3,State_4,State_5,State_6,nxtState_1,nxtState_2,nxtState_3,nxtState_4,nxtState_5,nxtState_6,p,R) % 2023-01-17 (updated 2023-11-17)
    ...
    [temporal_diff,yrnj,p,z_rnj_hidden,z_rnj_output] = calculateTemporalDiff(NN,p,State_1,State_2,State_3,State_4,State_5,State_6,nxtState_1,nxtState_2,nxtState_3,nxtState_4,nxtState_5,nxtState_6,R); % calculate the current temporal difference error (2023-05-24)
...
function [temporal_diff,yrnj,p,z_rnj_hidden,z_rnj_output] = calculateTemporalDiff(NN,p,State_1,State_2,State_3,State_4,State_5,State_6,nxtState_1,nxtState_2,nxtState_3,nxtState_4,nxtState_5,nxtState_6,R) % updated 2023-06-04
    ...
    p = updateAverageReward(p,temporal_diff); % update the approximated average reward
